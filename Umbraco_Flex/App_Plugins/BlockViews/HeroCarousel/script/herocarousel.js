angular.module("umbraco").controller("heroSlider",
            ...
            $scope.vm = {
                ...
                init: function () {
                    $scope.$watch(
                        function (){
                            var key = '';
                            if ($scope.block.data.items.contentData !== undefined 
                                && $scope.block.data.items.contentData.length != 0) 
                            {
                                $scope.block.data.items.contentData.forEach(
                                    function (data, index) {
                                        var mediaKey = '';
                                        var action = '';
                                        try {
                ...
                load: function () {
                    ...
                    try {
                        if ($scope.vm.loading == 0 
                            && $scope.block.data.items.contentData !== undefined 
                            && $scope.block.data.items.contentData.length != 0) 
                        {
                            $scope.vm.slides = [];
                            $scope.block.data.items.contentData.forEach(
